...
export class PrimaryStoreService {
  ...
  async getItem(id: string) {
    ...
    if (!this.canViewPrimaryItem(item)){
  ...
  async updateItem(id: string, updateItemDto: UpdateItemDto, userId: string) {
    ...
    if (!this.canModifyPrimaryItem(item, userId)) {
  ...
  private canViewPrimaryItem(item: (Item & { createdBy: { username:string, email:string, role:string } })) {
    return item.createdBy.role === UserRole.PRIMARY;
  }

  private canModifyPrimaryItem(item: (Item & { createdBy: User }), userId:string) {
    return item.createdBy.id === userId && item.createdBy.role === UserRole.PRIMARY
  }
